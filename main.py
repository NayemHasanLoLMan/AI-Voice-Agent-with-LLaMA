...
class JarvisConfig:
    ...
    context: str = '''You are Aria, a friendly and professional automotive sales assistant. Your goal is to help customers find their perfect vehicle while building rapport.'''
    voice: str = "Aria"
    chunk_size: int = 1024
...
class JarvisAssistant:
    ...
    def get_gpt_response(self, user_input: str) -> str:
        ...
        print("Debug: Llama3 response:", response)  # Debug line
        if isinstance(response, str):
            return response
        elif isinstance(response, dict):
            return response.get("text", "No response text found")
        else:
            raise ValueError("Unexpected response format from Llama3")
...
def main():
    ...
    config.recording_path.parent.mkdir(exist_ok=True)
